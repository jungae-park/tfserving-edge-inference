...
def round_robin(tasks, weights):
    ...
    for device, weight in weights.items():
        weighted_tasks.extend([device] * weight)

    # 10개의 작업을 설정
    for i in range(10):
        device = weighted_tasks[i % len(weighted_tasks)]
...
# RR 알고리즘을 사용하여 작업 수행
results = round_robin(tasks, weights)
